...
const DesignerTemplate = ({ data }) => {
  ...
  return (
    <Layout>
      <Seo
        title={data.strapiDesigner.name}
      />
      <div className="pb-2 border-b border-black">
        <h2 className="font-normal text-4xl">{data.strapiDesigner.name}</h2>
        <div className="flex flex-row space-x-4 align-items-center sans-serif mt-2">
          ...
          {data.strapiDesigner.twitter ?
            <Link to={data.strapiDesigner.twitter} className="flex items-center space-x-2 no-underline">
